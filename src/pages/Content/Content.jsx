...
const content = () => {
  ...
  async function getIdDictionary(index) {
    try {
      ...
      const response = await axios.get(`${process.env.API_URL}/users/${userId}/dictionaries/${dictionaryId}`, getToken)
  ...
  async function deleteIdDictionary(index) {
    try {
      const dictionaryId = dictionaries[index]._id;
      const response = await axios.delete(`${process.env.API_URL}/users/${userId}/dictionaries/${dictionaryId}`, getToken);
      getDictionaries();
  ...
  async function getDictionaries() {
    try {
      const response = await axios.get(`${process.env.API_URL}/users/${userId}/dictionaries`, getToken);
  ...
  async function getWords() {
    try {
      const response = await axios.get(`${process.env.API_URL}/users/${userId}/dictionaries/${dictionaryId}/words`, getToken);
  ...
  async function createWord() {
    try {
      ...
      await axios.post(`${process.env.API_URL}/users/${userId}/dictionaries/${dictionaryId}/words`, data, getToken);
  ...
  async function getOneWord(wordId) {
    try {
      ...
      await axios.get(`${process.env.API_URL}/users/${userId}/dictionaries/${dictionaryId}/words/${wordId}`, getToken);
    } catch (err) {
      console.log(err);
    }
  }
  async function deleteWords() {
    try {
      await Promise.all(selectedCheckbox.map(async (wordId) => {
        await axios.delete(`${process.env.API_URL}/users/${userId}/dictionaries/${dictionaryId}/words/${wordId}`, getToken)
      })
      );
      getWords();
      setSelectedCheckbox([]);
